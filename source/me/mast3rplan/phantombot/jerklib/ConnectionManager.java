...
public final class ConnectionManager
{
            ...
            if (selector.selectNow() > 0)
            {
                ...
                while (it.hasNext())
                {
                    ...
                    } catch (CancelledKeyException ke)
                    {
                        ...
                        com.gmt2001.Console.out.println("ConnectionManager: " + ke.getMessage());
                    }
                }
            }
        } catch (IOException e)
        {
            com.gmt2001.Console.out.println("ConnectionManager: " + e.getMessage());
        }
    }
    ...
    void notifyWriteListeners()
    {
        ...
        {
            wRequests.forEach(listener::receiveEvent);
        }
    }
        ...
        synchronized (sessionMap)
        {
            for (Session session : sessionMap.values())
            {
                ...
                if (state == State.DISCONNECTED && !session.isClosing)
                {
                    ...
                    long reconnectIntervalCur = (long) (30 * 1000);
                    int reconTriesShort = 10 * 2;
                    int reconTriesCur = reconTriesShort;

                    if (session.getRetries() >= reconTriesShort)
                    {
                        int reconTriesMed = 30 / 2;
                        if (session.getRetries() < (reconTriesShort + reconTriesMed))
                        {
                            reconnectIntervalCur = (long) (2 * 60 * 1000);
                            reconTriesCur = reconTriesMed;
                        } else
                        {
                            reconnectIntervalCur = (long) (5 * 60 * 1000);
                            reconTriesCur = ((3 * 60) + 20) / 5;
                        }
                    }
