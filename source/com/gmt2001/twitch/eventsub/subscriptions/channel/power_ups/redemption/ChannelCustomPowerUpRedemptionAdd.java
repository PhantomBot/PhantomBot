...
import tv.phantombot.event.eventsub.channel.power_ups.redemption.EventSubChannelCustomPowerUpRedemptionAddEvent;

/**
 * A viewer has redeemed a custom power up reward on the specified channel
 *
 * @author gmt2001
 */
public final class ChannelCustomPowerUpRedemptionAdd extends EventSubSubscriptionType {

    public static final String TYPE = "channel.custom_power_up_redemption.add";
     ...
     */
    public ChannelCustomPowerUpRedemptionAdd() {
        super();
        this.subscribe();
    }
     ...
     * @param e The event
     */
    public ChannelCustomPowerUpRedemptionAdd(EventSubInternalNotificationEvent e) {
     ...
     */
    public ChannelCustomPowerUpRedemptionAdd(String broadcaster_user_id) {
        super();
    ...
    protected void onEventSubInternalNotificationEvent(EventSubInternalNotificationEvent e) {
        try {
            if (e.subscription().type().equals(ChannelCustomPowerUpRedemptionAdd.TYPE)) {
                EventSub.debug(ChannelCustomPowerUpRedemptionAdd.TYPE);
                EventBus.instance().postAsync(new EventSubChannelCustomPowerUpRedemptionAddEvent(new ChannelCustomPowerUpRedemptionAdd(e)));
            }
        } catch (Exception ex) {
    ...
    protected boolean isMatch(EventSubSubscription subscription) {
        return subscription.type().equals(ChannelCustomPowerUpRedemptionAdd.TYPE)
