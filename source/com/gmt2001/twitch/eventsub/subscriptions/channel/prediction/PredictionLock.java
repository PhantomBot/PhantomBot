...
import tv.phantombot.event.eventsub.channel.prediction.EventSubPredictionLockEvent;
 ...
 * @author gmt2001
 */
public final class PredictionLock extends EventSubSubscriptionType {

    public static final String TYPE = "channel.prediction.lock";
     ...
     * @param e The event
     */
    public PredictionLock(EventSubInternalNotificationEvent e) {
        ...
        this.sLocked_at = e.event().getString("locked_at");
        this.started_at = EventSub.parseDate(this.sStarted_at);
        this.locked_at = EventSub.parseDate(this.sLocked_at);
     ...
     */
    public PredictionLock(String broadcaster_user_id) {
        super();
    ...
    protected EventSubSubscription proposeSubscription() {
        return this.proposeSubscriptionInternal(PredictionLock.TYPE, PredictionLock.VERSION,
    ...
    protected void onEventSubInternalNotificationEvent(EventSubInternalNotificationEvent e) {
        try {
            if (e.subscription().type().equals(PredictionLock.TYPE)) {
                EventSub.debug(PredictionLock.TYPE);
                EventBus.instance().postAsync(new EventSubPredictionLockEvent(new PredictionLock(e)));
            }
        } catch (Exception ex) {
    ...
    protected boolean isMatch(EventSubSubscription subscription) {
        return subscription.type().equals(PredictionLock.TYPE)
