...
/**
 * The osuToMsnAdapter class intercepts application messages sent from the rhythm game osu! to the MSN live messenger by
...
public class osuToMsnAdapter {
    ...
    public static final MyUser32 lib = MyUser32.INSTANCE;
    public static String _msnString;
    public void osuToMsnAdapter() 
    { 
        try {
            msnListener.start();
            msnListener.join();
    ...
    public interface MyUser32 extends User32 {
        ...
        public static final MyUser32 INSTANCE = (MyUser32) Native.loadLibrary("user32", MyUser32.class, W32APIOptions.UNICODE_OPTIONS);
    ...
    Thread msnListener = new Thread(new Runnable() {
                public void run() { 
                    synchronized(obj) {
                        WNDCLASSEX wx = new WNDCLASSEX();
                        wx.lpszClassName = lpClassName;
                        wx.lpfnWndProc = WNDPROC;
                        if (lib.RegisterClassEx(wx).intValue() > 0) {
                            // Create a native window
                            HWND hWnd = lib.CreateWindowEx(0, lpClassName, "", 0,
                                    0, 0, 0, 0, null, null, null, null);
                            // Register the callback
                            try {
                                // Use SetWindowLongPtr if available (64-bit safe)
                                lib.SetWindowLongPtr(hWnd, GWL_WNDPROC, WNDPROC);
                                //System.err.println("Registered 64-bit callback");
                            } catch(UnsatisfiedLinkError e) {
                                // Use SetWindowLong if SetWindowLongPtr isn't available
                                lib.SetWindowLong(hWnd, GWL_WNDPROC, WNDPROC);
                            ...
                            MSG msg = new MSG();
                            msg.clear();
                            while(lib.GetMessage(msg, hWnd, 0, 0) > 0) {
