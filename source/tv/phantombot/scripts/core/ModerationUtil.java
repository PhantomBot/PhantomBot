 ...
 * This doesn't allow us to make functions to get "totals" of things and reuse
 * them.
 * 
 * @author ScaniaTV
 */
public class ModerationUtil {
	...
	private final Pattern GRAPHEMES_PATTERN = Pattern.compile(
			"\\p{M}|\\p{So}|\\p{InPhonetic_Extensions}|\\p{InLetterlikeSymbols}|\\p{InDingbats}|\\p{InBoxDrawing}|\\p{InBlockElements}|\\p{InGeometricShapes}|\\p{InHalfwidth_and_Fullwidth_Forms}");

	/**
	 * Types of filters we have,
	 */
	public static enum FilterType {
		Links, Caps, Symbols, Spam, Zalgo, OneManSpam, Digits, FakePurges, Actions, Paragraphs, Emotes, Graphemes, R9k
	}
	...
	public boolean hasURL(String message, boolean doDecipher, boolean isSongrequestsEnabled) {
		// If fake links like "google dot com", "google(.)com" should be changed to
		// "google.com".
		if (doDecipher) {
	...
	/**
	 * Method that deciphers a URL from a message, if a link has "(dot)" and not a
...
//				JSONArray list = blacklist.getJSONArray("list");
	...
	public boolean hasMaximumNonAlphanumeric(String message, int maxNonAlpha, float maxPercent) {
		...
		for (int i = 0; i < message.length(); i++) {
			...
			if (!(Character.isLetterOrDigit(c) || Character.isWhitespace(c))) {
				...
				if (totalNonAlphanumericCount >= maxNonAlpha
						|| ((totalNonAlphanumericCount / messageLength) >= maxPercent)) {
	...
	/**
	 * Method that checks if multiple non-alphanumeric characters (random ones) are
	 * repeating multiple times in a row.
	 * 
	 * @param message
	 * @param maxLength
	 * @return
	...
	/**
	 * Method that checks if a word is repeated multiple times in a row or in a
	 * phrase
	 * 
	 * @param message
	 * @param maxLength       How many times a word can be repeated after another in
	 *                        a row.
	 * @param maxTimes        How many times total in a message a word can be
	 *                        repeated.
	 ...
	 * @param timeFrame   - How many messages a user is allowed to send in a time
	 *                    frame. (Milliseconds)
	 * @param maxMessages - How many messages the user can send in our time frame.
	 * @param maxSames    - How many of the same messages the user can send in the
	 *                    time frame (one after another).
	 * @return
	 */
	...
	/**
	 * Method that checks if a message has a special letter format (font) and other
	 * ASCII art.
	 * 
	 * @param message
	 * @param maxPercent
	...
	/**
	 * Method that matches the similarity between the last few messages. (This is to
	 * stop bots).
	 * 
	 * @param message
	 * @param maxSimilarity
