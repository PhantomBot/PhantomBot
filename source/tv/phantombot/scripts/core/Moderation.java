...
//	private final ScheduledExecutorService chatCacheHandler = Executors.newSingleThreadScheduledExecutor();
	...
	public Moderation() {
		...
		/*
		 * String[] keys =
		 * PhantomBot.instance().getDataStore().GetKeyList("chatModerator_Beta", "");
		 * for (String key : keys) { String value =
		 * PhantomBot.instance().getDataStore().get("chatModerator_Beta", key);
		 * 
		 * if (key.startsWith("bool_")) { // boolean settings.put(key.substring(5),
		 * Boolean.parseBoolean(value)); } else if (key.startsWith("int_")) { // Int.
		 * settings.put(key.substring(4), Integer.parseInt(value)); } else if
		 * (key.startsWith("float_")) { // Float. settings.put(key.substring(6),
		 * Float.parseFloat(value)); } else { // String. settings.put(key, value); } }
		 */
	...
	public void onIrcModerationMessage(IrcModerationEvent event) {
		 ...
		 * event.getSession().sayNow(".timeout " + event.getSender() + " 10"); return; }
		 * if (hasSpecialLetters(event.getMessage(), (float)0.10)) {
		 * event.getSession().sayNow(".timeout " + event.getSender() + " 10"); return; }
		 * if (hasMaximumCaps(event.getMessage(), 10,
		 * (float)settings.get("float_maxCapsPercent"), event.getTags().get("emotes")))
	 ...
	 * @param session
	 */
	private void punishUser(FilterType type, String username, String reason, FilterType filterType,
	 ...
	 */
	private void deleteUser(String username, String message, String messageID, FilterType filterType,
			PunishmentType punishmentType, TwitchSession session) {
	 ...
	 * @param username       - The user being punished.
	 * @param message        - The message said in chat to the user.
	 * @param reason         - The reason for the timeout, this is shown with the
	 *                       ban reason message from Twitch.
	 * @param punishmentType - The type of punishment being applied to the user.
	 * @param seconds        - How long the timeout will last on the user.
	 * @param filterType     - The type of filter applied to the user.
	 * @param session        - The Twitch session to send messages.
	 */
	private void timeoutUser(String username, String message, String reason, int seconds, FilterType filterType,
			PunishmentType punishmentType, TwitchSession session) {
	 ...
	 * @param session        - The Twitch session to send messages.
	 */
	private void banUser(String username, String message, String reason, FilterType filterType,
			PunishmentType punishmentType, TwitchSession session) {
	 ...
	 * @param session        - The Twitch session to send messages.
	 */
	private void sendPunishMessage(String username, String message, FilterType filterType,
			PunishmentType punishmentType, TwitchSession session) {
		long currentTime = System.currentTimeMillis();

		if (!(boolean) getSetting(filterType.name() + "IsSilent", false)
				&& (currentTime - lastPunishmentMessage) <= (long) getSetting("PunishmentMessageTimeout", 5000)) {
	...
	private void cleanChatCache() {
		int maxChatCacheSize = PhantomBot.instance().getDataStore().GetInteger("chatModeration", "",
				"maxChatCacheSize");
		int keepMessageTime = (PhantomBot.instance().getDataStore().GetInteger("chatModeration", "",
				"oneManSpamKeepTime") * 1000);
		long currentTime = System.currentTimeMillis();
