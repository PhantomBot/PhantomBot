...
import tv.phantombot.event.webpanel.websocket.WebPanelSocketConnectedEvent;
...
public class PanelSocketServer extends WebSocketServer {
    ...
    private void handleMessage(WebSocket webSocket, String jsonString) {
        ...
        try {
            if (jsonObject.has("command")) {
                ...
                handleConnection(jsonObject.has("query_id") ? jsonObject.getString("query_id") : "");
                return;
     ...
     * @param id Optional unique ID which is sent back to the WebSocket.
     */
    private void handleConnection(String id) {
        if (!id.isEmpty()) {
            JSONStringer jsonObject = new JSONStringer();
            jsonObject.object().key("query_id").value(id).endObject();
            webSocket.send(jsonObject.toString());
        ...
        EventBus.postAsync(new WebPanelSocketConnectedEvent());
    }
