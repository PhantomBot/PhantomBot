...
public class ConfigurationManager {
	...
	static Properties getConfiguration() {
		...
		if (startProperties.getProperty(PROP_USER) == null || startProperties.getProperty(PROP_OAUTH) == null
				|| startProperties.getProperty(PROP_CHANNEL) == null) {
		 ...
		 */
		requiredPropertiesErrorMessage = String.join(" ", Arrays.stream(requiredProperties)
				.filter(x -> startProperties.getProperty(x) == null).toArray(String[]::new));
	...
	private static Boolean generateDefaultValues(Properties startProperties) {
		...
		changed |= setDefaultIfMissing(startProperties, PROP_PANEL_USER, PANEL_STANDARD_USER,
				"No Panel Username, using default value of '" + PANEL_STANDARD_USER
						+ "' for Control Panel and YouTube Player");
		...
		changed |= setDefaultIfMissing(startProperties, PROP_PANEL_PASSWORD, PANEL_STANDARD_PASSWORD,
				"No Panel Password, using default value of '" + PANEL_STANDARD_PASSWORD
						+ "' for Control Panel and YouTube Player");
	 ...
	 */
	private static Boolean setDefaultIfMissing(Properties properties, String propertyName, String defaultValue,
	 ...
	 *         to be generated
	 */
	private static Boolean setDefaultIfMissing(Properties properties, String propertyName,
	...
	public static Boolean getBoolean(Properties properties, String propertyName, Boolean defaulValue) {
		...
		try {
			...
			com.gmt2001.Console.err.println("[Error] could not load property '" + propertyName
					+ "'. Fallback to default value (" + defaulValue + ")");
		}
	...
	private static void doSetup(Properties startProperties) {
		try {
			...
			com.gmt2001.Console.out
					.print("If you're running the bot on a server, make sure to open the following ports: \r\n");
			com.gmt2001.Console.out.print(
