...
public class StreamElementsCache extends Cache {
	...
	public void run() {
		try {
			...
			com.gmt2001.Console.debug
					.println("StreamElementsCache.run: Failed to execute initial sleep [InterruptedException]: "
							+ ex.getMessage());
		}
	...
	private void updateCache() throws Exception {
			...
			if (jsonResult.getInt("_http") == 200) {
				...
				if (jsonResult.has("error") && jsonResult.getString("error").equalsIgnoreCase("Unauthorized")) {
					com.gmt2001.Console.err.println(
							"StreamElementsCache.updateCache: Bad JWT token disabling the StreamElements module.");
					PhantomBot.instance().getDataStore().SetString("modules", "", "./handlers/streamElementsHandler.js",
							"false");
					this.stop();
				} else {
		...
		if (firstUpdate && this.isRunning()) {
			firstUpdate = false;
			EventBus.post(new StreamElementsDonationInitializedEvent());
		}

		if (donations != null && this.isRunning()) {
			for (int i = 0; i < donations.length(); i++) {
				if (cache == null || !cache.containsKey(donations.getJSONObject(i).getString("_id"))) {
					EventBus.postAsync(new StreamElementsDonationEvent(donations.getJSONObject(i).toString()));
				}
			}
		}
