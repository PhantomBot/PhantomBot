...
public class UsernameCache extends Cache {
			...
			if (user.getBoolean("_success")) {
				if (user.getInt("_http") == 200) {
					if (user.getJSONArray("users").length() > 0) {
						String displayName = user.getJSONArray("users").getJSONObject(0).getString("display_name")
								.replaceAll("\\\\s", " ");
						String userID = user.getJSONArray("users").getJSONObject(0).getString("_id");
						cache.put(username, new UserData(displayName, userID));
					}
				} else {
					com.gmt2001.Console.debug.println("UsernameCache.updateCache: Failed to get username [" + username
							+ "] http error [" + user.getInt("_http") + "]");
				}
			} else {
	...
	public boolean exists(String userName) {
		// Check the cache first, if the user doesn't exist call the API and check the
		// cache again.
	...
	/*
	 * Internal object for tracking user data. Note that while Twitch represents the
	 * userID as a String, it is an integer value. We define this as an int here to
	 * conserve memory usage. The maximum value of an unsigned int within Java is
	 * 4,294,967,295 which should serve as a large enough data type.
	 */
