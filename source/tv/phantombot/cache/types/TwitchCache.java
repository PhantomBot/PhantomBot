 ...
 * TwitchCache Class
 *
 * This class keeps track of certain Twitch information such as if the channel
 * is online or not and sends events to the JS side to indicate when the channel
 * has gone off or online.
 */
public class TwitchCache extends Cache {
	 ...
	 */
	public TwitchCache(String channel) {
	 ...
	 * manage retrieving data from the Twitch API. This loop runs every 30 seconds,
	 * querying data from Twitch.
	 */
	@Override
	public void run() {
		// If this cache starts before the database, we need to wait.
		// TODO find a way without freezing the thread!
	...
	/**
	 * Polls the Clips endppint, trying to find the most recent clip. Note that
	 * because Twitch reports by the viewcount, and has a limit of 100 clips, it is
	 * possible to miss the most recent clip until it has views.
	 *
	 * We do not throw an exception because this is not a critical function unlike
	 * the gathering of data via the updateCache() method.
	 */
	private void updateClips() throws JSONException {
		...
		String discordDoClipsCheck = PhantomBot.instance().getDataStore().GetString("discordSettings", "",
				"clipsToggle");
		if ((doCheckClips == null || doCheckClips.equals("false"))
				&& (discordDoClipsCheck == null || discordDoClipsCheck.equals("false"))) {
			return;
		}
...
//		String createdAt = "";
		...
		if (clipsObj.has("clips")) {
			...
			if (clipsData.length() > 0) {
				setDBString("most_viewed_clip_url",
						"https://clips.twitch.tv/" + clipsData.getJSONObject(0).getString("slug"));
...
//						createdAt = clipData.getString("created_at");
		...
		if (clipURL.length() > 0) {
			...
			EventBus.postAsync(new TwitchClipEvent(clipURL, creator, title, thumbnailObj));
		}
	}

	/**
	 * Polls the Twitch API and updates the database cache with information. This
	 * method also sends events when appropriate.
	 */
	private void updateCache() throws Exception {
		...
		try {
			...
			if (streamObj.getBoolean("_success") && streamObj.getInt("_http") == 200) {
				...
				if (!this.isOnline && isOnline) {
					this.isOnline = true;
					EventBus.postAsync(new TwitchOnlineEvent());
					sentTwitchOnlineEvent = true;
				} else if (this.isOnline && !isOnline) {
					this.isOnline = false;
					EventBus.postAsync(new TwitchOfflineEvent());
				}
				...
				if (isOnline) {
					...
					try {
						...
						this.streamUptimeSeconds = (long) (Math
								.floor(currentDate.getTime() - streamCreatedDate.getTime()) / 1000);
						this.streamCreatedAt = streamObj.getJSONObject("stream").getString("created_at");
					} catch (Exception ex) {
						success = false;
						com.gmt2001.Console.err.println(
								"TwitchCache::updateCache: Bad date from Twitch, cannot convert for stream uptime ("
										+ streamObj.getJSONObject("stream").getString("created_at") + ")");
					}

					/* Determine the preview link. */
					this.previewLink = streamObj.getJSONObject("stream").getJSONObject("preview").getString("template")
		...
		try {
			...
			if (streamObj.getBoolean("_success") && streamObj.getInt("_http") == 200) {
					...
					if (!streamObj.isNull("game")) {
						...
						if (!forcedGameTitleUpdate && !this.gameTitle.equals(gameTitle)) {
							...
							if (!sentTwitchOnlineEvent) {
								this.gameTitle = gameTitle;
								EventBus.postAsync(new TwitchGameChangeEvent(gameTitle));
							}
							this.gameTitle = gameTitle;
						}
				...
				if (new File("./web/panel").isDirectory() && streamObj.has("display_name")
						&& !streamObj.isNull("display_name")) {
				...
				if (streamObj.has("status")) {
					if (!streamObj.isNull("status")) {
						...
						if (!forcedStreamTitleUpdate && !this.streamTitle.equals(streamTitle)) {
							...
							if (!sentTwitchOnlineEvent) {
								this.streamTitle = streamTitle;
								EventBus.postAsync(new TwitchTitleChangeEvent(streamTitle));
							}
							this.streamTitle = streamTitle;
						}
						if (forcedStreamTitleUpdate && this.streamTitle.equals(streamTitle)) {
	...
	/**
	 * Returns a String representation of true/false to indicate if the stream is
	 * online or not.
	 */
	...
	/**
	 * Gets a string from the database. Simply a wrapper around the PhantomBot
	 * instance.
	 *
	...
	/**
	 * Sets a string into the database. Simply a wrapper around the PhantomBot
	 * instance.
	 *
	 * @param String The database key to use for inserting the value into the
	 *               streamInfo table.
