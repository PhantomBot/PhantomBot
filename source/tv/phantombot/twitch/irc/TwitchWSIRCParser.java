...
public class TwitchWSIRCParser implements Runnable {
	...
	private final UsernameCache usernameCache = Cache.getUsername();
	...
	public static synchronized TwitchWSIRCParser instance(WebSocket webSocket, String channelName,
			...
			try {
				...
				if (bulkSubscriberGifters.containsKey(tags.get("login"))) {
					...
					if (tags.get("login").equalsIgnoreCase(ANONYMOUS_GIFTER_TWITCH_USER)) {
						scriptEventManager.onEvent(
								new TwitchAnonymousSubscriptionGiftEvent(tags.get("msg-param-recipient-user-name"),
										tags.get("msg-param-months"), tags.get("msg-param-sub-plan")));
					} else {
						scriptEventManager.onEvent(new TwitchSubscriptionGiftEvent(tags.get("login"),
								tags.get("msg-param-recipient-user-name"), tags.get("msg-param-months"),
								tags.get("msg-param-sub-plan")));
					}
				}
	...
	private void parseLine(String rawMessage) {
		...
		if (messageParts[0].startsWith("@")) {
			...
			// We want to skip the first element which are badges, since they are parsed
			// already.
	...
	/**
	 * ---------------------------------------------------------------------- Event
	 * Handling Methods. The below methods are all referenced from the parserMap
	 * object.
	 * ----------------------------------------------------------------------
	 */
	...
	private void onPrivMsg(String message, String username, Map<String, String> tags) {
		...
		if (tags.containsKey("subscriber") && tags.get("subscriber").equals("1")) {
			EventBus.postAsync(
					new IrcPrivateMessageEvent(session, "jtv", "SPECIALUSER " + username + " subscriber", tags));
		}
	...
	private void onNotice(String message, String username, Map<String, String> tags) {
		...
		case "Login authentication failed":
			...
			com.gmt2001.Console.out.println(
					"Please see: https://community.phantombot.tv/t/twitch-indicates-the-oauth-password-is-incorrect");
		...
		case "Invalid NICK":
			...
			com.gmt2001.Console.out
					.println("Twitch Inidicated Invalid Bot Name. Check 'user=' setting in botlogin.txt");
			com.gmt2001.Console.out.println("Exiting PhantomBot.");
			com.gmt2001.Console.out.println();
			PhantomBot.exitError();
			break;
	...
	private void onUserNotice(String message, String username, Map<String, String> tags) {
		if (tags.containsKey("msg-id")) {
			if (tags.get("msg-id").equalsIgnoreCase("resub")) {
				scriptEventManager.onEvent(new TwitchReSubscriberEvent(tags.get("login"),
						tags.get("msg-param-cumulative-months"), tags.get("msg-param-sub-plan")));
			} else if (tags.get("msg-id").equalsIgnoreCase("sub")) {
				if (tags.get("msg-param-sub-plan").equalsIgnoreCase("Prime")) {
					scriptEventManager.onEvent(
							new TwitchPrimeSubscriberEvent(tags.get("login"), tags.get("msg-param-cumulative-months")));
				} else {
					scriptEventManager.onEvent(new TwitchSubscriberEvent(tags.get("login"),
							tags.get("msg-param-sub-plan"), tags.get("msg-param-cumulative-months")));
				}
				...
				bulkSubscriberGifters.put(tags.get("login"), new SubscriberBulkGifter(tags.get("login"),
						Integer.parseInt(tags.get("msg-param-mass-gift-count")), false));

				// Send event for this.
				if (tags.get("login").equalsIgnoreCase(ANONYMOUS_GIFTER_TWITCH_USER)) {
					scriptEventManager.onEvent(new TwitchMassAnonymousSubscriptionGiftedEvent(
							tags.get("msg-param-mass-gift-count"), tags.get("msg-param-sub-plan")));
				} else {
					scriptEventManager.onEvent(new TwitchMassSubscriptionGiftedEvent(tags.get("login"),
							tags.get("msg-param-mass-gift-count"), tags.get("msg-param-sub-plan")));
				}
				...
				// https://discuss.dev.twitch.tv/t/anonymous-sub-gifting-to-launch-11-15-launch-details/18683
				bulkSubscriberGifters.put(tags.get("login"), new SubscriberBulkGifter(tags.get("login"),
						Integer.parseInt(tags.get("msg-param-mass-gift-count")), true));
				...
				if (tags.get("msg-id").equalsIgnoreCase("raid")) {
					scriptEventManager
							.onEvent(new TwitchRaidEvent(tags.get("login"), tags.get("msg-param-viewerCount")));
				}
			}
		}
	}
		...
		if (tags.containsKey("user-type")) {
			if (tags.get("user-type").length() > 0) {
				...
				if (channelName.equals(username)) {
					...
					com.gmt2001.Console.out
							.println("[ERROR] Please go to http://twitchapps.com/tmi and generate a new token.");
					com.gmt2001.Console.out.println(
							"[ERROR] Be sure to go to twitch.tv and login as the bot before getting the token.");
					com.gmt2001.Console.out.println(
							"[ERROR] After, open the botlogin.txt file and replace the oauth= value with the token.");
					com.gmt2001.Console.out.println();
				} else {
					com.gmt2001.Console.out.println();
					com.gmt2001.Console.out.println("[ERROR] " + username + " is not detected as a moderator!");
					com.gmt2001.Console.out
							.println("[ERROR] You must add " + username + " as a channel moderator for it to chat.");
					com.gmt2001.Console.out.println(
							"[ERROR] Type /mod " + username + " to add " + username + " as a channel moderator.");
					com.gmt2001.Console.out.println();
