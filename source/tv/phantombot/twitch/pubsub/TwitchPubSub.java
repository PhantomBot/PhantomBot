...
public class TwitchPubSub {
	 ...
	 *
	 * @param {string} channel Name of the channel to start the instance on. As of
	 *                 right now you can onyl start one instance.
	...
	private TwitchPubSub(String channel, int channelId, int botId, String oAuth) {
		...
		try {
			this.twitchPubSubWS = new TwitchPubSubWS(new URI("wss://pubsub-edge.twitch.tv"), this, channelId, botId,
					oAuth);
	...
	/**
	 * Try to reconnect to the PubSub websocket when the connection is closed with
	 * some logic.
	 */
	...
	private class TwitchPubSubWS extends WebSocketClient {
		 ...
		 *
		 * @param {boolean} reconnect Changes the console log message from connection to
		 *                  reconnecting.
		 * @return {Boolean}
		 */
		public Boolean connectWSS(Boolean reconnect) {
			if (!reconnect) {
				com.gmt2001.Console.debug
						.println("Connecting to Twitch PubSub-Edge (SSL) [" + this.uri.getHost() + "]");
			} else {
				com.gmt2001.Console.debug
						.println("Reconnecting to Twitch PubSub-Edge (SSL) [" + this.uri.getHost() + "]");
			}
		...
		/**
		 * Used to start the ping timer for PubSub. Since PubSub does not send pings, we
		 * need to requests them to keep our connection opened. We will send a PING
		 * request every 4.9 minutes. Twitch recommends every 5 minutes.
		 */
		...
		/**
		 * This purges the ping timer. It is used when the user tries to connect with a
		 * bad oauth token.
		 */
		...
		/**
		 * This function parses the message we get from PubSub. Since everything is sent
		 * in a jsonObject there is a bit of checks to do.
		 *
				...
				if (dataObj.has("message")) {
					...
					if (messageObj.has("data")) {
						...
						if (data.has("moderation_action") && data.has("args") && data.has("created_by")) {
							...
							if (timeoutCache.containsKey(data.getString("target_user_id"))
									&& (timeoutCache.get(data.getString("target_user_id"))
							...
							case "delete":
								...
								EventBus.postAsync(new PubSubModerationDeleteEvent(args1, creator, args2));
								break;
							case "timeout":
								this.log(args1 + " has been timed out by " + creator + " for " + args2 + " seconds. "
										+ (args3.length() == 0 ? "" : "Reason: " + args3));
								EventBus
										.postAsync(new PubSubModerationTimeoutEvent(args1, creator,
												(messageCache.containsKey(args1.toLowerCase())
														? messageCache.get(args1.toLowerCase())
														: ""),
												args3, args2));
								break;
							case "untimeout":
								this.log(args1 + " has been un-timed out by " + creator + ".");
								EventBus.postAsync(new PubSubModerationUnTimeoutEvent(args1, creator));
								break;
							case "ban":
								this.log(args1 + " has been banned by " + creator + ". "
										+ (args2.length() == 0 ? "" : "Reason: " + args2));
								EventBus
										.postAsync(new PubSubModerationBanEvent(args1, creator,
												(messageCache.containsKey(args1.toLowerCase())
														? messageCache.get(args1.toLowerCase())
														: ""),
												args2));
							...
							case "unban":
								...
								EventBus.postAsync(new PubSubModerationUnBanEvent(args1, creator));
								break;
							case "mod":
							...
							case "denied_twitchbot_message":
								this.log(creator + " denied a message from " + args1 + ". Message id: "
										+ data.getString("msg_id") + ".");
								break;
							case "approved_twitchbot_message":
								this.log(creator + " allowed a message from " + args1 + ". Message id: "
										+ data.getString("msg_id") + ".");
								break;
							}
						}
		...
		private void log(String message) {
			if (PhantomBot.instance().getDataStore().GetString("chatModerator", "", "moderationLogs").equals("true")) {
				Logger.instance().log(Logger.LogType.Moderation,
						"[" + Logger.instance().logTimestamp() + "] " + message);
			}
		}

		/**
		 * Handles the event of when the socket opens, it also sends the login
		 * information and the topics we can to listen to.
		 */
		@Override
		...
		/**
		 * Handles the event of when the socket closes, this will also attempt to
		 * reonnect to PubSub when it happens.
		 *
		...
		public void onClose(int code, String reason, boolean remote) {
			com.gmt2001.Console.debug
					.println("Code [" + code + "] Reason [" + reason + "] Remote Hangup [" + remote + "]");
		...
		/**
		 * Handles the error event we can get from the socket. It will also print it in
		 * the console.
		 *
		...
		/**
		 * Class for the PING timer. Since PubSub doesn't send PINGS we need to request
		 * them.
		 */
